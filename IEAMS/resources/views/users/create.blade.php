...
<div class="max-w-3xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-bold tracking-tight text-white">Create New User</h2>
            <p class="text-sm text-slate-400 mt-1">Register a new system user and configure their role-based permissions.</p>
        </div>
        ...
        <form action="{{ route('users.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. Abdullah Al Mamun" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Email -->
                <div class="space-y-1.5">
                    <label for="email" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Email Address *</label>
                    <input type="email" name="email" id="email" required value="{{ old('email') }}" placeholder="e.g. mamun@nha.gov.bd" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="password" name="password" id="password" required placeholder="Minimum 8 characters" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="password" name="password_confirmation" id="password_confirmation" required placeholder="Re-type password" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="employee_id" id="employee_id" value="{{ old('employee_id') }}" placeholder="e.g. EMP1002" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition font-mono uppercase">
                </div>

                <div class="space-y-1.5">
                    <label for="phone" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Phone / Mobile</label>
                    <input type="text" name="phone" id="phone" value="{{ old('phone') }}" placeholder="e.g. 01712345678" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition">
                </div>

                <div class="space-y-1.5">
                    <label for="nid" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">NID Number</label>
                    <input type="text" name="nid" id="nid" value="{{ old('nid') }}" placeholder="e.g. 1995123456789" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <p class="text-[10px] text-slate-500 mt-1">Allowed formats: JPG, JPEG, PNG (Max 2MB)</p>
                </div>
            ...
            <div class="space-y-4 p-4 bg-slate-900/30 border border-slate-800/80 rounded-2xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                        ...
                        <textarea name="present_address" id="present_address" rows="3" placeholder="Enter present residential address" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition">{{ old('present_address') }}</textarea>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="permanent_address" id="permanent_address" rows="3" placeholder="Enter permanent home address" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition">{{ old('permanent_address') }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    @foreach($roles as $role)
                        <div class="p-4 rounded-xl bg-[#080B11] border border-slate-800/80 hover:border-indigo-500/50 transition cursor-pointer relative flex items-start">
                            <div class="flex items-center h-5">
                                <input type="checkbox" name="roles[]" value="{{ $role->id }}" id="role_{{ $role->id }}" class="w-4 h-4 text-indigo-600 bg-slate-900 border-slate-700 rounded focus:ring-indigo-500" {{ (is_array(old('roles')) && in_array($role->id, old('roles'))) ? 'checked' : '' }}>
                            </div>
...
<script>
    ...
    function loadBranchesForOffice(officeId, selectedBranchId = null) {
        ...
        fetch(`/api/offices/${officeId}/branches`)
            ...
            .then(data => {
                ...
                if (selectedBranchId) {
                    loadDepartmentsForBranch(selectedBranchId, "{{ old('department_id') }}");
                }
            })
            .catch(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const officeVal = document.getElementById('office_id').value;
        if (officeVal) {
            loadBranchesForOffice(officeVal, "{{ old('branch_id') }}");
        }
    });
</script>
@endsection
