...
<div class="max-w-3xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-bold tracking-tight text-white">Add Floor to Building</h2>
            <p class="text-sm text-slate-400 mt-1">Register a building floor along with its maximum planned apartment capacity.</p>
        </div>
    ...
    <div class="p-8 rounded-2xl bg-[#0E1325]/80 border border-slate-800/80 shadow-xl">
        <form action="{{ route('property.floors.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="floor_number" id="floor_number" required value="{{ old('floor_number') }}" placeholder="e.g. Ground Floor, 1st Floor, 0, 1" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Floor Name -->
                <div class="space-y-1.5">
                    <label for="floor_name" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Floor Name/Alias</label>
                    <input type="text" name="floor_name" id="floor_name" value="{{ old('floor_name') }}" placeholder="e.g. Floor 1" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="number" name="total_apartment" id="total_apartment" required value="{{ old('total_apartment', 4) }}" placeholder="e.g. 4" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
            </div>
