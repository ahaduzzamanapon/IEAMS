...
<div class="max-w-3xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-bold tracking-tight text-white">Edit Project Plot</h2>
            <p class="text-sm text-slate-400 mt-1">Modify registered plot details under NHA project land specification.</p>
        </div>
    ...
    <div class="p-8 rounded-2xl bg-[#0E1325]/80 border border-slate-800/80 shadow-xl">
        <form action="{{ route('property.plots.update', $plot->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <select name="project_id" id="project_id" required onchange="loadLandsForProject(this.value)" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        ...
                        @foreach($projects as $proj)
                            <option value="{{ $proj->id }}" {{ old('project_id', $plot->project_id) == $proj->id ? 'selected' : '' }}>
                                {{ $proj->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="land_id" id="land_id" required class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        ...
                        @foreach($lands as $l)
                            <option value="{{ $l->id }}" {{ old('land_id', $plot->land_id) == $l->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="plot_number" id="plot_number" required value="{{ old('plot_number', $plot->plot_number) }}" placeholder="e.g. PLOT-01A" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Plot Name -->
                <div class="space-y-1.5">
                    <label for="plot_name" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Plot Name</label>
                    <input type="text" name="plot_name" id="plot_name" value="{{ old('plot_name', $plot->plot_name) }}" placeholder="e.g. Premium Residential Plot A" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" step="0.01" name="plot_area" id="plot_area" required value="{{ old('plot_area', $plot->plot_area) }}" placeholder="e.g. 3600.00" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Status -->
                <div class="space-y-1.5">
                    <label for="status" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Status *</label>
                    <select name="status" id="status" required class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        <option value="vacant" {{ old('status', $plot->status) == 'vacant' ? 'selected' : '' }}>Vacant</option>
                        <option value="sold" {{ old('status', $plot->status) == 'sold' ? 'selected' : '' }}>Sold</option>
                        <option value="leased" {{ old('status', $plot->status) == 'leased' ? 'selected' : '' }}>Leased</option>
                    </select>
                </div>
            </div>
