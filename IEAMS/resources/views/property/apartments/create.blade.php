...
<div class="max-w-3xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-bold tracking-tight text-white">Register Apartment</h2>
            <p class="text-sm text-slate-400 mt-1">Register flat specifications and occupancy metrics on a selected floor level.</p>
        </div>
    ...
    <div class="p-8 rounded-2xl bg-[#0E1325]/80 border border-slate-800/80 shadow-xl">
        <form action="{{ route('property.apartments.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="plot_id" required onchange="loadBuildingsForPlot(this.value)" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-2.5 py-2 text-[11px] text-white focus:outline-none focus:border-indigo-500 transition">
                        <option value="">Select Land first</option>
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="apartment_number" id="apartment_number" required value="{{ old('apartment_number') }}" placeholder="e.g. Apt-101, A-05-03" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Apartment Name -->
                <div class="space-y-1.5">
                    <label for="name" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Apartment Name *</label>
                    <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. Standard Deluxe Suite 101" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" step="0.01" name="size" id="size" required value="{{ old('size') }}" placeholder="e.g. 1250.00" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Orientation -->
                <div class="space-y-1.5">
                    <label for="orientation" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Orientation *</label>
                    <input type="text" name="orientation" id="orientation" required value="{{ old('orientation') }}" placeholder="e.g. North-East, South, West" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="bedrooms" id="bedrooms" value="{{ old('bedrooms', 3) }}" placeholder="e.g. 3" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Bathrooms -->
                <div class="space-y-1.5">
                    <label for="bathrooms" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Bathrooms</label>
                    <input type="number" name="bathrooms" id="bathrooms" value="{{ old('bathrooms', 2) }}" placeholder="e.g. 2" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>

                <!-- Balcony -->
                <div class="space-y-1.5">
                    <label for="balcony" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Balcony count</label>
                    <input type="number" name="balcony" id="balcony" value="{{ old('balcony', 2) }}" placeholder="e.g. 2" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="parking" id="parking" value="{{ old('parking') }}" placeholder="e.g. Spot-101" class="w-full bg-[#080B11] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            ...
            <div class="p-4 rounded-xl bg-[#080B11] border border-slate-800/80 flex items-start">
                <div class="flex items-center h-5">
                    <input type="checkbox" name="utility_connection" value="1" id="utility_connection" class="w-4 h-4 text-indigo-600 bg-slate-900 border-slate-700 rounded focus:ring-indigo-500" {{ old('utility_connection') ? 'checked' : '' }}>
                </div>
    ...
    function loadLandsForProject(projectId, selectedLandId = null) {
        ...
        fetch(`/api/projects/${projectId}/lands`)
            ...
            .then(data => {
                ...
                if (selectedLandId) {
                    loadPlotsForLand(selectedLandId, "{{ old('plot_id') }}");
                }
            })
            .catch(() => {
    ...
    function loadPlotsForLand(landId, selectedPlotId = null) {
        ...
        fetch(`/api/lands/${landId}/plots`)
            ...
            .then(data => {
                ...
                if (selectedPlotId) {
                    loadBuildingsForPlot(selectedPlotId, "{{ old('building_id') }}");
                }
            })
            .catch(() => {
        ...
        fetch(`/api/plots/${plotId}/buildings`)
            ...
            .then(data => {
                ...
                if (selectedBuildingId) {
                    loadFloorsForBuilding(selectedBuildingId, "{{ old('floor_id') }}");
                }
            })
            .catch(() => {
