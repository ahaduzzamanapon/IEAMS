...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        @if($asset->asset_type !== 'consumer')
            const qrText = `Asset ID: {{ $asset->unique_asset_id }} | Type: {{ ucfirst($asset->asset_type) }} | Category: {{ $asset->category->name }} | Sub-Category: {{ $asset->subCategory->name }} | Brand/Model: {{ $asset->brand }} {{ $asset->model }} | Serial: {{ $asset->serial_number }} | Purchase Cost: TK {{ number_format($asset->purchase_cost, 2) }} | Capitalized Cost: TK {{ number_format($asset->capitalized_cost, 2) }} | Total Cost: TK {{ number_format($asset->total_cost, 2) }} | PO: {{ $asset->purchase_order_number ?? 'N/A' }} | Invoice: {{ $asset->invoice_number ?? 'N/A' }} | Vendor: {{ $asset->vendor ? $asset->vendor->name : 'N/A' }} | Warranty: {{ $asset->warranty_applicable ? 'Yes (Expires: ' . \Carbon\Carbon::parse($asset->warranty_end_date)->format('d-m-Y') . ')' : 'No' }}`;
